...
@bp_converte_setor_estagiario_pdf.route('/api/setor/estagiar/pdf', methods=['POST'])
def converte_setor_estagiarios_pdf():
    try:
        body = request.json or {}
        setor_nome = body.get('setor')  # Nome do setor a ser filtrado
        ...
        if not estagiarios:
            conexao.close()
            return jsonify({'erro': f'Nenhum estagiário encontrado no setor {setor_nome}'}), 404
        ...
        for estagiario in estagiarios:
            ...
            nome_base = f"FREQUENCIA_{nome_limpo.replace(' ', '_')}_{estagiario['id']}"
